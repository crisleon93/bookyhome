...
export default function PostLogin() {
  ...
  // Funciones auxiliares para nivel de fidelización
  const getNextLevel = (currentLevel) => {
    const levels = { 'Bronce': 'Plata', 'Plata': 'Oro', 'Oro': 'Platino', 'Platino': 'Platino' };
    return levels[currentLevel] || 'Plata';
  };

  const getPointsForNextLevel = (totalGastado) => {
    if (totalGastado < 50000) return Math.ceil((50000 - totalGastado) / 1000);
    if (totalGastado < 150000) return Math.ceil((150000 - totalGastado) / 1000);
    if (totalGastado < 300000) return Math.ceil((300000 - totalGastado) / 1000);
    return 0;
  };
    ...
    try {
      ...
      try {
        ...
        setNivelFidelizacion({ 
          nivel: resEstadisticas.data.nivel_fidelizacion || 'Bronce', 
          puntos: Math.floor((resEstadisticas.data.total_gastado || 0) / 1000),
          siguiente_nivel: getNextLevel(resEstadisticas.data.nivel_fidelizacion),
          puntos_para_siguiente: getPointsForNextLevel(resEstadisticas.data.total_gastado || 0)
        });
      } catch (error) {
        console.error('Error cargando estadísticas:', error);
        setEstadisticas({ total_gastado: 0, num_compras: 0, libro_mas_comprado: null });
        setCategoriasFavoritas([]);
        setNivelFidelizacion({ nivel: 'Bronce', puntos: 0, siguiente_nivel: 'Plata', puntos_para_siguiente: 50 });
      }
