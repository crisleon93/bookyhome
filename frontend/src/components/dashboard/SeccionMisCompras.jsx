    ...
    <>
      ...
      <div className="pl-card" style={{ padding: "2rem", marginBottom: 20 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "10px", flexWrap: "wrap" }}>
            ...
            {totalPendientes > 0 && (
              <button
                ...
                disabled={limpiandoPendientes}
                title="Cancela las órdenes que quedaron pendientes de pago sin completar"
                style={{
                  display: "inline-flex", alignItems: "center", gap: "6px",
                  background: limpiandoPendientes ? "#f3f4f6" : "#FFFBEB",
                  color: "#B45309",
                  ...
                  fontSize: "0.82rem",
                  fontWeight: 700,
                  cursor: limpiandoPendientes ? "not-allowed" : "pointer",
                  opacity: limpiandoPendientes ? 0.7 : 1,
                  transition: "all 0.2s"
                }}
              >
        ...
        <div style={{
          ...
          {/* Entregadas */}
          <div
            onClick={() => { setFiltroEstado('entregado'); setPaginaActual(1); }}
            style={{
          ...
          {/* En camino */}
          <div
            onClick={() => { setFiltroEstado('camino'); setPaginaActual(1); }}
            style={{
          ...
          {/* Canceladas */}
          <div
            onClick={() => { setFiltroEstado('cancelado'); setPaginaActual(1); }}
            style={{
      ...
      {ordenesLoading ? (
        ...
        <div className="pl-card" style={{ padding: 0, overflow: "hidden", borderRadius: "16px", border: "1.5px solid #ebe5dc", boxShadow: "0 2px 10px rgba(0,0,0,0.03)", background: "#fff" }}>
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "left" }}>
              <thead>
                <tr style={{ background: "#fbf9f6", borderBottom: "1.5px solid #ebe5dc", color: "#8c857b", fontSize: "0.72rem", fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.06em" }}>
                ...
                {ordenesPaginadas.map((orden) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td style={{ padding: "16px 18px", whiteSpace: "nowrap" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                          <span style={{
                            ...
                            background: estadoVisible.completada ? "#ECFDF5" : estadoVisible.esCancelada ? "#FEF2F2" : estadoVisible.pago === "Pendiente de pago" ? "#FFFBEB" : "#EFF6FF",
                            display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0,
                            border: `1px solid ${estadoVisible.completada ? "#A7F3D0" : estadoVisible.esCancelada ? "#FECACA" : estadoVisible.pago === "Pendiente de pago" ? "#FDE68A" : "#BFDBFE"}`
                          }}>
                          ...
                          <div>
                            <strong style={{ fontSize: "0.94rem", color: "#1F2937", display: "block" }}>
                      ...
                      {/* 2. FECHA */}
                      <td style={{ padding: "16px 18px", whiteSpace: "nowrap", color: "#4B5563", fontSize: "0.86rem", fontWeight: 600 }}>
                        {orden.fecha ? new Date(orden.fecha).toLocaleDateString("es-CO") : "—"}
                      </td>

                      {/* 3. PRODUCTOS */}
                      <td style={{ padding: "16px 18px", color: "#374151", fontSize: "0.86rem" }}>
                        <span style={{
                          display: "inline-flex",
                          alignItems: "center",
                          gap: "5px",
                        ...
                        {orden.items?.[0]?.titulo && (
                          <div style={{ fontSize: "0.76rem", color: "#6B7280", marginTop: "4px", maxWidth: "200px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                            {orden.items[0].titulo}
                          </div>
                        )}
                      </td>
                      ...
                      <td style={{ padding: "16px 18px", textAlign: "right", whiteSpace: "nowrap" }}>
                        <span style={{ color: "#7A1E3A", fontSize: "1.05rem", fontWeight: 800, letterSpacing: "-0.01em" }}>
                          ${formatNumber(orden.total)} COP
                        </span>
                      </td>

                      {/* 7. ACCIONES — 3 slots fijos para alineación vertical perfecta */}
                      <td style={{ padding: "16px 18px", textAlign: "center", whiteSpace: "nowrap" }}>
                        <div style={{ display: "grid", gridTemplateColumns: "90px 120px 120px", gap: "8px", justifyContent: "center", alignItems: "center" }}>
                          ...
                          <div>
                            {estadoVisible.pago === "Pendiente de pago" ? (
                              <button
                                onClick={() => {
                                  const targetId = orden.id_orden_db || orden.id_orden;
                                  navigate(`/?seccion=Carrito&pagarOrden=${targetId}&metodo=${encodeURIComponent(orden.metodo_pago || 'nequi')}&entrega=${encodeURIComponent(orden.tipo_entrega || 'domicilio')}`, {
                                    state: {
                                      autoPayOrderId: targetId,
                                      autoPayMethod: orden.metodo_pago,
                                      autoPayEntrega: orden.tipo_entrega || 'domicilio',
                                      ordenObj: orden
                                    }
                                  });
                                }}
                                ...
                                onMouseLeave={(e) => { e.currentTarget.style.background = "#FDF2F4"; e.currentTarget.style.color = "#7A1E3A"; }}
                              >
                                <span>💳</span> Pagar
                              </button>
                            ) : null}
                          ...
                          {/* Slot 2: Ver Detalles — siempre visible */}
                          <div>
                            <button
                              onClick={() => handleAbrirDetalle(orden)}
                              style={{
                                width: "100%",
                                padding: "6px 0",
                                fontSize: "0.78rem",
                            ...
                            {estadoVisible.pago === "Pagado" ? (
                              <button
                                ...
                                style={{
                                  ...
                                  justifyContent: "center",
                                  gap: "5px",
                                  boxShadow: "0 2px 6px rgba(122,30,58,0.22)",
                                  transition: "all 0.15s ease"
                                }}
          ...
          >
              ...
              <div>
                <div style={{ display: "flex", alignItems: "center", gap: "10px", flexWrap: "wrap" }}>
                  <h2 style={{ margin: 0, fontSize: "1.35rem", fontWeight: 800, color: "#0F172A" }}>
                    Orden #{idVisible(modalDetalleOrden)}
                  </h2>
                  <span style={{
                    fontSize: "0.78rem",
                    fontWeight: 800,
            ...
            {modalDetalleOrden.tipo_entrega === "retiro_tienda" ? (
              <div style={{
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "14px", flexWrap: "wrap", gap: "10px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                    ...
                    <strong style={{ fontSize: "0.98rem", color: "#1E3A8A" }}>Click & Collect · Retiro en Tienda</strong>
                  </div>
                  {/* PIN DE RETIRO DESTACADO */}
                  <div style={{
                    background: "#0F172A",
                    color: "#FDE047",
                    padding: "4px 14px",
                ...
                <div style={{ fontSize: "0.88rem", color: "#334155", lineHeight: "1.55", display: "grid", gap: "6px" }}>
                  <div>
                    <strong>Librería:</strong> {modalDetalleOrden.tienda_retiro?.nombre_tienda || "Librería Asociada"}
                  </div>
                  {modalDetalleOrden.tienda_retiro?.direccion && (
                    <div>
                      <strong>Dirección:</strong> 📍 {modalDetalleOrden.tienda_retiro.direccion}
                    </div>
                  )}
                  {modalDetalleOrden.tienda_retiro?.telefono && (
                    <div>
                      <strong>Teléfono:</strong> 📞 {modalDetalleOrden.tienda_retiro.telefono}
                    </div>
                  )}
                  {modalDetalleOrden.fecha_limite_retiro && (
                    <div>
                      <strong>Plazo de retiro:</strong> Hasta el {new Date(modalDetalleOrden.fecha_limite_retiro).toLocaleDateString("es-CO", { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })}
                    </div>
                  )}
                </div>

                {/* Estado presencial */}
                <div style={{ marginTop: "14px", paddingTop: "12px", borderTop: "1px solid #DBEAFE", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "10px" }}>
                  <div style={{ fontSize: "0.82rem" }}>
                    {(modalDetalleOrden.estado === "pagado" || modalDetalleOrden.estado_orden === "pagado") ? (
                      <span style={{ color: "#15803D", fontWeight: 700 }}>
                        ✓ Pago completado. Presenta tu PIN en el mostrador para retirar tus libros.
                      </span>
                    ) : modalDetalleOrden.estado_retiro === "habilitado_pago" ? (
                      <span style={{ color: "#B45309", fontWeight: 700 }}>
                        🔔 Pago habilitado por el vendedor en la librería.
                      </span>
                    ) : modalDetalleOrden.estado_retiro === "en_tienda" ? (
                      <span style={{ color: "#1E40AF", fontWeight: 700 }}>
                        📍 Notificaste tu llegada. El encargado te atenderá en breve.
                      </span>
                    ) : (
                      <span style={{ color: "#64748B" }}>
                        🟡 Libros apartados en la librería física.
                      </span>
                    )}
                  </div>

                  {/* Acciones en tienda dentro del modal */}
                  {modalDetalleOrden.estado_retiro === "reservado" && modalDetalleOrden.estado !== "pagado" && modalDetalleOrden.estado_orden !== "pagado" && (
                    <button
                      onClick={async () => {
                        await handleLlegadaTienda(modalDetalleOrden.id_orden_db || modalDetalleOrden.id_orden);
                        handleAbrirDetalle(modalDetalleOrden);
                      }}
                      disabled={notificandoId === (modalDetalleOrden.id_orden_db || modalDetalleOrden.id_orden)}
                      style={{
                        padding: "6px 12px",
                        fontSize: "0.78rem",
                        borderRadius: "6px",
                  ...
                  {modalDetalleOrden.estado_retiro === "habilitado_pago" && modalDetalleOrden.estado !== "pagado" && modalDetalleOrden.estado_orden !== "pagado" && (
                    <button
                      onClick={() => {
                        handleCerrarDetalle();
                        const targetId = modalDetalleOrden.id_orden_db || modalDetalleOrden.id_orden;
                        navigate(`/?seccion=Carrito&pagarOrden=${targetId}&metodo=${encodeURIComponent(modalDetalleOrden.metodo_pago || 'nequi')}&entrega=retiro_tienda`, {
                          state: {
                            autoPayOrderId: targetId,
                            autoPayMethod: modalDetalleOrden.metodo_pago,
                            autoPayEntrega: 'retiro_tienda',
                            ordenObj: modalDetalleOrden
                          }
                        });
                      }}
                      style={{
              ...
              <div style={{
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "8px" }}>
                  <span>🚚</span>
                  <strong style={{ fontSize: "0.95rem", color: "#1E293B" }}>Envío a Domicilio</strong>
                </div>
                {modalDetalleOrden.envio?.numero_guia ? (
                  <div style={{ fontSize: "0.85rem", color: "#475569" }}>
                    Guía: <strong>{modalDetalleOrden.envio.numero_guia}</strong> ({modalDetalleOrden.envio.empresa_mensajeria})
                    <br />
                    Estado: {modalDetalleOrden.envio.estado_envio || "En tránsito"}
                  </div>
                ) : (
            ...
            <div style={{ marginBottom: "20px" }}>
              <h4 style={{ margin: "0 0 12px", fontSize: "0.95rem", fontWeight: 800, color: "#1E293B" }}>
                Productos ({modalDetalleOrden.items?.length || 0})
              </h4>
              <div style={{ display: "grid", gap: "10px" }}>
                {modalDetalleOrden.items?.map((item, idx) => (
                  <div key={idx} style={{
                    ...
                    <div>
                      <div style={{ fontWeight: 700, fontSize: "0.88rem", color: "#0F172A" }}>{item.titulo}</div>
                      <div style={{ fontSize: "0.78rem", color: "#64748B" }}>
                        {item.autor_libro || "Autor"} · Cantidad: {item.cantidad}
                      </div>
                    </div>
                    <div style={{ fontWeight: 700, fontSize: "0.92rem", color: "#7A1E3A" }}>
                      {formatCurrency(Number(item.precio_libro || item.precio_final || 0) * Number(item.cantidad || 1))}
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div style={{ borderTop: "1px solid #E2E8F0", paddingTop: "16px", marginBottom: "20px", display: "grid", gap: "8px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.88rem", color: "#64748B" }}>
                <span>Método de pago</span>
                <span style={{ fontWeight: 600, color: "#0F172A" }}>{modalDetalleOrden.metodo_pago || "En línea"}</span>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: "1.1rem", fontWeight: 800, color: "#0F172A", marginTop: "4px" }}>
                <span>Total</span>
                <span style={{ color: "#7A1E3A" }}>{formatCurrency(modalDetalleOrden.total)}</span>
              </div>
            </div>
            ...
            <div style={{ display: "flex", gap: "10px", justifyContent: "flex-end" }}>
              {(modalDetalleOrden.estado === "pagado" || modalDetalleOrden.estado_orden === "pagado") && (
                <button
                  onClick={() => {
                    handleCerrarDetalle();
                    handleVerBaucher(modalDetalleOrden);
                  }}
              ...
              <>
                ...
                <div style={{ background: "#fcfaf7", padding: "14px 16px", borderRadius: "12px", marginBottom: "14px", border: "1px solid #e0dbd4" }}>
                  <div style={{ display: "grid", gap: "8px", marginBottom: "10px" }}>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between" }}>
                      <span style={{ color: "#666", fontSize: "0.9rem" }}>Forma de Entrega</span>
                      <span style={{ fontWeight: 700, color: ordenSeleccionada.tipo_entrega === 'retiro_tienda' ? '#1E40AF' : '#374151' }}>
